...
public record BatchProcessingResult
{
    ...
    public string GetSummary()
    {
        ...
        if (CrackedCount > 0)
        {
            parts.Add($"{CrackedCount} cracked");
        }

        if (ZippedCount > 0)
        {
            parts.Add($"{ZippedCount} zipped");
        }
        ...
        if (CrackFailedCount > 0)
        {
            parts.Add($"{CrackFailedCount} crack failed");
        }

        if (ZipFailedCount > 0)
        {
            parts.Add($"{ZipFailedCount} zip failed");
        }

        if (UploadFailedCount > 0)
        {
            parts.Add($"{UploadFailedCount} upload failed");
        }

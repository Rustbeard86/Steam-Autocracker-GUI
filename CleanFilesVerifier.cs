...
public static class CleanFilesVerifier
{
    public static VerificationResult VerifyCleanInstallation(string gamePath)
    {
        ...
        try
        {
            ...
            {
                "ali213.ini", "valve.ini", "hlm.ini", "steam_interfaces.txt", "ColdClientLoader.ini",
                "SmartSteamEmu.ini", "SmartSteamEmu64.dll", "SmartSteamEmu.dll", "ALI213.ini", "steam_api.ini",
                "steam_appid.txt", // This one is tricky - could be legit or crack
                "local_save.txt", "achievements.json", "controller.vdf", "items.json", "stats.txt"
            };
                ...
                foreach (var file in files)
                {
                    ...
                    if (indicator == "steam_appid.txt")
                    {
                        ...
                        if (result.HasSteamSettings || result.HasBackupFiles)
                        {
                            ...
                            result.ContaminationReasons.Add(
                                "Found suspicious steam_appid.txt alongside crack artifacts");
                        }
                    }
                    else
                    {
            ...
            {
                "*Lobby*.lnk", "*Multiplayer*.lnk", "*LAN*.lnk", "*Online*.lnk", "*_MP.lnk", "*_Lobby.lnk"
            };
            ...
            foreach (var pattern in shortcutPatterns)
            {
                ...
                foreach (var shortcut in shortcuts)
                {
                    ...
                    if (IsOurShortcut(shortcut))
                    {
                        ...
                        result.ContaminationReasons.Add(
                            $"Found auto-generated lobby shortcut: {Path.GetFileName(shortcut)}");
                    }
                }
            }
            ...
            foreach (var apiFile in steamApiFiles)
            {
                ...
                foreach (var file in files)
                {
                    ...
                    if (IsKnownCrackSize(fileInfo.Length, Path.GetFileName(file)))
                    {
                        ...
                        result.FoundCrackArtifacts.Add(file);
                        result.ContaminationReasons.Add(
                            $"Steam API file has suspicious size: {Path.GetFileName(file)}");
                    }
                }
            }
            ...
            var debugFiles = new[]
            {
                "debug_log.txt", "steam_api.log", "goldberg_steam.log", "SteamEmu.log", "SmartSteamEmu.log"
            };

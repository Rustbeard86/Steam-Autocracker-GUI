...
public static class UploadStatusManager
{
        ...
        if (e.ColumnIndex == grid.Columns["UploadStatus"]?.Index && e.RowIndex >= 0)
        {
            ...
            if (!string.IsNullOrEmpty(uploadUrl))
            {
                ...
                if (!string.IsNullOrEmpty(gameName))
                {
                    ...
                    {
                        var searchUrl =
                            $"https://cs.rin.ru/forum/search.php?keywords={Uri.EscapeDataString(gameName)}&terms=all&author=&sc=1&sf=titleonly&sk=t&sd=d&sr=topics&st=0&ch=300&t=0&submit=Search";
    ...
    private static void ShowUploadToast(string gameName, string uploadUrl)
    {
        ...
        var panel = new Panel
        {
            Dock = DockStyle.Fill, BackColor = Color.FromArgb(0, 120, 0), Padding = new Padding(1)
        };

        var innerPanel = new Panel { Dock = DockStyle.Fill, BackColor = Color.FromArgb(20, 30, 20) };

        var iconLabel = new Label
        {
            Text = "✅", Font = new Font("Segoe UI", 20), Location = new Point(10, 20), Size = new Size(40, 40)
        };
        ...
        innerPanel.Controls.AddRange(iconLabel, textLabel, closeButton);

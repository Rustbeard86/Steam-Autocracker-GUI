...
public static class AutoCleanupSystem
{
    ...
    [
        ...
        "*Multiplayer*.lnk", "*LAN*.lnk", "*Online*.lnk", "launch_*.bat", // Batch files we might create
    ...
    private static async Task<CleanupResult> PerformAutomaticCleanup(string gamePath, string gameName, Form parentForm)
    {
        ...
        try
        {
            ...
            var crackFiles = new[]
            {
                "ALI213.ini", "valve.ini", "steam_api.ini", "steam_interfaces.txt", "local_save.txt",
                "achievements.json", "items.json", "stats.txt", "controller.vdf", "SmartSteamEmu.ini",
